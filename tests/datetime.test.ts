...
import { datetime, ERROR_CODES, validate } from "../src";
...
test("datetime - type", () => {
  ...
  ).toEqual({ date3: ERROR_CODES.type });
});
...
test("datetime - min/max", () => {
  ...
  expect(
    ...
    date2: ERROR_CODES.datetime.max,
    date3: ERROR_CODES.type,
  });
  ...
  expect(
    ...
    date2: ERROR_CODES.datetime.min,
    date3: ERROR_CODES.type,
  });
});
